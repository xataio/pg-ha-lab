#!/usr/bin/env bash
# sync04: the sync01 geometry (sync any/1 + quorum, {primary + sync standby}
# trapped) with the async02 twist: the single-replica operator is re-pinned onto
# the primary's node first, so it is trapped too.
#
# EXPECTED: during the fault, identical to sync01 (the operator was
# quorum-denied anyway): no promotion, fence at ~cut+213s, outage until
# heal. At heal, the async02 race occurs but should be DEFANGED by sync+quorum:
# the returning operator either (a) loses the race to the resuming primary
# (sync01 outcome), or (b) promotes — in which case the quorum check must deny
...
scenario::init sync04-operator-trapped
