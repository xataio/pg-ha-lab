#!/usr/bin/env bash
# s05: same fault as s01 (async config, primary's node fully isolated), but
...
#
# EXPECTED (vs s01, which differs only in operator placement): NO promotion
...
# dual-ack — the availability/consistency inversion of s01.
set -euo pipefail
